...
class HippoRAG:
    def __init__(
        ...
        self.global_config._normalize_provider_config()
        ...
        llm_label = self.global_config.llm_runtime_label()
        embedding_label = self.global_config.embedding_runtime_label()
        self.working_dir = os.path.join(
            self.global_config.save_dir, f"{llm_label}__{embedding_label}"
        )
        legacy_working_dir = os.path.join(
            self.global_config.save_dir,
            f"{self.global_config.llm_name.replace('/', '_')}_{self.global_config.embedding_model_name.replace('/', '_')}",
        )
        if not os.path.exists(self.working_dir) and os.path.exists(legacy_working_dir):
            logger.info(f"Using legacy working directory: {legacy_working_dir}")
            self.working_dir = legacy_working_dir
        ...
        self.openie_results_path = os.path.join(
            self.global_config.save_dir,
            f"openie_results_ner_{self.global_config.llm_runtime_label()}.json",
        )
        legacy_openie_results_path = os.path.join(
            self.global_config.save_dir,
            f"openie_results_ner_{self.global_config.llm_name.replace('/', '_')}.json",
        )
        if not os.path.exists(self.openie_results_path) and os.path.exists(
            legacy_openie_results_path
        ):
            logger.info(f"Using legacy OpenIE cache file: {legacy_openie_results_path}")
            self.openie_results_path = legacy_openie_results_path
