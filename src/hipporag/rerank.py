...
class DSPyFilter:
    ...
    def make_template(self, dspy_file_path):
        if dspy_file_path is not None:
            ...
            try:
                default_prompt_path = resources.files(
                    "hipporag.prompts.dspy_prompts"
                ).joinpath("filter_llama3.3-70B-Instruct.json")
                with default_prompt_path.open("r", encoding="utf-8") as prompt_file:
                    dspy_saved = json.load(prompt_file)
            except Exception:
